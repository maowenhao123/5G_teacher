...
@implementation SJFastForwardView
...
- (void)setRate:(CGFloat)rate {
    if ( rate != _rate ) {
        ...
        NSAttributedString *text = [NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol>  _Nonnull make) {
            make.append([NSString stringWithFormat:@"%.01fx", rate]).textColor(sources.fastForwardRateTextColor).font(sources.fastForwardRateTextFont);
            make.append(sources.fastForwardFFText).font(sources.fastForwardFFTextFont).textColor(sources.fastForwardFFTextColor);
        }];
...
- (void)_updateSettings {
    ...
    for ( CAShapeLayer *layer in _triangles ) {
        layer.strokeColor = layer.fillColor = sources.fastForwardTriangleColor.CGColor;
    }
}
@end
